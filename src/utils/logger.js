...
const LOG_LEVEL =
	LOG_LEVELS[(process.env.LOG_LEVEL || process.env.REACT_APP_LOG_LEVEL || "").toUpperCase()] ||
	LOG_LEVELS.WARN;
...
class Logger {
	...
	constructor(context, level) {
		if (!context) {
			throw new Error("Logger cannot be used without providing context");
		}

		this.context = context;
		this.level = (level && LOG_LEVELS[level]) || LOG_LEVEL;
	}
	...
	_log(level, exceptionCode, message, args) {
		...
		if (isBrowser()) {
			return args.length > 0
				? console[BROWSER_LOGGER[level] || level](message, ...args)
